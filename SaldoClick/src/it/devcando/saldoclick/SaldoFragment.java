...
public class SaldoFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		bar_entrate_layout = (RelativeLayout) rootView
				.findViewById(R.id.bar_entrate_layout);
		bar_uscite_layout = (RelativeLayout) rootView
				.findViewById(R.id.bar_uscite_layout);

		Bundle bundle = getActivity().getIntent().getExtras();
		final Conto conto = (Conto) bundle.getParcelable("conto");
		textViewContabile.setText(String.valueOf(conto.getSaldo_contabile()));
		textViewDisponibile
				.setText(String.valueOf(conto.getSaldo_disponibile()));
		textViewNumeroConto.setText(conto.getCodice_iban());
		textViewIntestatario.setText(conto.getIntestatario());
		entrate = conto.getEntrate();
		uscite = conto.getUscite();

		arrow_up = getActivity().getResources().getDrawable(
				R.drawable.expander_close_holo_light);
		arrow_down = getActivity().getResources().getDrawable(
				R.drawable.expander_open_holo_light);
		movimenti_list_layout = (LinearLayout) rootView
				.findViewById(R.id.movimenti_layout);
		movimenti_arrow = (ImageView) rootView
				.findViewById(R.id.movimenti_arrow);
		movimenti_header = (TextView) rootView
				.findViewById(R.id.movimenti_label);
			...
			public void onClick(View v) {
				if (isMovimentiVisible == false) {
					if (movimenti_list_layout.getChildCount() == 2) {
						...
						for (Iterator<Movimento> i = conto.getMovimenti()
								.iterator(); i.hasNext();) {
							...
							if (!i.hasNext()) {
								convertView.findViewById(R.id.separator_light)
										.setVisibility(View.GONE);
							}
						}
					} else {
						// We already have added the movimentos...just show them
						for (int i = 1; i < movimenti_list_layout
								.getChildCount(); i++) {
							// Start from 2 because 0 is the textview
							movimenti_list_layout.getChildAt(i).setVisibility(
									View.VISIBLE);
						}
					}
	...
	public void drawChart(double entrate, double uscite) {
		...
		label.setText(getResources().getString(R.string.entrate_label) + " "
				+ entrate + " €");
		...
		RelativeLayout.LayoutParams layoutParamsLabel = (RelativeLayout.LayoutParams) label
				.getLayoutParams();
		...
		label_u.setLayoutParams(new RelativeLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
		RelativeLayout.LayoutParams layoutParamsLabelU = (RelativeLayout.LayoutParams) label_u
				.getLayoutParams();
	...
	public double normalize(double x, int max, double max_norm) {
		// Let's use as maximum value the sum of income and outcome! I think
		// it's ok anyway! :) :)
